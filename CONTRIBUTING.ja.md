...
- **ラベル:**
  新しい Issue には自動でラベル（種類・対象・深刻度）が付きます。
  バージョン・手順・ログが足りない不具合報告には、それをお願いするコメントが 1 回だけ付きます。
- **送るもの:**
  そのために、タイトルと本文の文章（コードブロック・表・翻訳の行・画像は除く）と、貼られたログのエラー行（パスのユーザー名は伏せます）を TypeSafe AI の分類モデルに送っています（フレームワークのリポジトリの [`tools/issue-triage.py`](https://github.com/TomXV/dragnwash-modframework/blob/main/tools/issue-triage.py)）。
